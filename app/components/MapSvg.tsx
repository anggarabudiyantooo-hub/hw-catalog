...
export function MapSvg({ w = 320, h = 176 }: { w?: number; h?: number }) {
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="map-svg" role="img" aria-label="Peta lokasi Kandang Jalu, Surakarta"
      ...
      <text x="16" y={h * 0.46} fontSize="9.5" letterSpacing="1.5" fill="#6f4a24" paintOrder="stroke" stroke="#efe3c4" strokeWidth="3">JALAN PAHLAWAN</text>
      <text x={w * 0.88} y={h * 0.62} fontSize="8.6" letterSpacing="1" fill="#6f4a24" transform={`rotate(90 ${w * 0.88} ${h * 0.62})`}>JL. KARTINI</text>
      ...
      <text x="14" y={h - 6} fontSize="9.5" fontStyle="italic" fill="#7a5a38">± 15 menit dari pusat Kota Solo</text>
    </svg>
  );
}
